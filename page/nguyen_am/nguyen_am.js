...
let vowel = ["អ","ា", "ិ", "ី", "ឹ", "ឺ", "ុ", "ូ", "ួ", "ើ", "ឿ", "ៀ", "េ", "ែ", "ៃ", "ោ", "ៅ", "ុំ", "ំ", "ាំ", "ះ","ិះ", "ុះ", "េះ", "ោះ"]
for ( i in vowel){
    l = i % 5 *104
    t =  (i-(i%5)) * 22
    ...
    s.style.backgroundImage = 'url(https://github.com/viaasana/tu_luyen_chu_viet_tieng_khmer_nam_bo/blob/main/image/phu_am/'+ i + '.png?raw=true)'
    //s.style.color = "aliceblue"
    //s.style.border = "2px solid black"
    s.style.left = l + 'px'
    s.style.top = t + 'px'
    //s.style.backgroundColor = "darkslateblue"
    s.innerHTML = '<h1>' + vowel[i] + '</h1><span class="pl_video_button" id="video_' +i + '" onclick=open_video_button('+ i + ')>&#9654;</span><span class="pl_sound_button" id="sound_" ' + i + ' onclick=open_sound_button(' + i + ')>🎧</span>'
...
function open_video_button(video_id){
    ...
    video.src ='https://github.com/viaasana/tu_luyen_chu_viet_tieng_khmer_nam_bo/blob/main/video/vowel_video/'+ video_id +'.mp4?raw=true';
    video.id = 'video'
    video.autoplay = true
...
function open_sound_button(sound_id){
    ...
    audio.src ='https://github.com/viaasana/tu_luyen_chu_viet_tieng_khmer_nam_bo/blob/main/sound/NGUYEN_AM/Recording%20(' + parseFloat(sound_id  +35)+ ').m4a?raw=true'
    audio.id = 'audio'
    audio.autoplay = true
